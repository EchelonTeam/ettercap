...
GtkItemFactoryEntry gmenu_start[] = {
   {N_("/_Start"),                NULL,          NULL,                 0, "<Branch>" },
   {N_("/Start/Start sniffing"),  "<control>w",  gtkui_start_sniffing, 0, "<StockItem>", GTK_STOCK_YES },
   {N_("/Start/Stop sniffing"),   "<control>e",  gtkui_stop_sniffing,  0, "<StockItem>", GTK_STOCK_NO },
   {"/Start/sep1",            NULL,          NULL,                 0, "<Separator>" },
   {N_("/Start/E_xit"),           "<control>x",  gtkui_exit,        0, "<StockItem>", GTK_STOCK_QUIT }
};

GtkItemFactoryEntry gmenu_targets[] = {
   {N_("/_Targets"),                 NULL,          NULL,                  0, "<Branch>" },
   {N_("/Targets/Current _Targets"), "t",           gtkui_current_targets, 0, "<StockItem>", GTK_STOCK_FIND },
   {N_("/Targets/Select TARGET(s)"), "<control>t",  gtkui_select_targets,  0, "<StockItem>", GTK_STOCK_ADD },
   {"/Targets/sep1",             NULL,          NULL,                  0, "<Separator>" },
   {N_("/Targets/_Protocol..."),     "p",           gtkui_select_protocol, 0, "<StockItem>", GTK_STOCK_JUMP_TO },
   {N_("/Targets/Reverse matching"), NULL,          toggle_reverse,        0, "<ToggleItem>" },
   {"/Targets/sep2",             NULL,          NULL,                  0, "<Separator>" },
   {N_("/Targets/_Wipe targets"),    "<shift>W",    wipe_targets,          0, "<StockItem>", GTK_STOCK_CLEAR }
};

GtkItemFactoryEntry gmenu_hosts[] = {
   {N_("/_Hosts"),                  NULL,         NULL,             0, "<Branch>" },
   {N_("/Hosts/_Hosts list"),       "h",          gtkui_host_list,  0, "<StockItem>", GTK_STOCK_INDEX },
   {"/Hosts/sep1",              NULL,         NULL,             0, "<Separator>" },
   {N_("/Hosts/_Scan for hosts"),   "<control>s", gtkui_scan,       0, "<StockItem>", GTK_STOCK_FIND },
   {N_("/Hosts/Load from file..."), NULL,         gtkui_load_hosts, 0, "<StockItem>", GTK_STOCK_OPEN },
   {N_("/Hosts/Save to file..."),   NULL,         gtkui_save_hosts, 0, "<StockItem>", GTK_STOCK_SAVE }
};

GtkItemFactoryEntry gmenu_view[] = {
   {N_("/_View"),                        NULL, NULL,                   0, "<Branch>" },
   {N_("/View/_Connections"),      "<shift>C", gtkui_show_connections, 0, "<StockItem>", GTK_STOCK_JUSTIFY_FILL },
   {N_("/View/Pr_ofiles"),         "<shift>O", gtkui_show_profiles,    0, "<StockItem>", GTK_STOCK_JUSTIFY_LEFT },
   {N_("/View/_Statistics"),              "s", gtkui_show_stats,       0, "<StockItem>", GTK_STOCK_PROPERTIES },
   {"/View/sep1",                    NULL, NULL,                   0, "<Separator>" },
   {N_("/View/Resolve IP addresses"),    NULL, toggle_resolve,         0, "<ToggleItem>" },
   {N_("/View/_Visualization method..."), "v", gtkui_vis_method,       0, "<StockItem>", GTK_STOCK_PREFERENCES },
   {N_("/View/Visualization _regex..."),  "R", gtkui_vis_regex,        0, "<StockItem>", GTK_STOCK_FIND },
   {"/View/sep1",                    NULL, NULL,                   0, "<Separator>" },
   {N_("/View/Set the _WiFi key..."),      "w", gtkui_wifi_key,          0, "<StockItem>", GTK_STOCK_FIND }
};

GtkItemFactoryEntry gmenu_mitm[] = {
   {N_("/_Mitm"),                    NULL, NULL,                0, "<Branch>" },
   {N_("/Mitm/Arp poisoning..."),    NULL, gtkui_arp_poisoning, 0, "<Item>" },
   {N_("/Mitm/Icmp redirect..."),    NULL, gtkui_icmp_redir,    0, "<Item>" },
   {N_("/Mitm/Port stealing..."),    NULL, gtkui_port_stealing, 0, "<Item>" },
   {N_("/Mitm/Dhcp spoofing..."),    NULL, gtkui_dhcp_spoofing, 0, "<Item>" },
   {"/Mitm/sep1",                NULL, NULL,                0, "<Separator>" },
   {N_("/Mitm/Stop mitm attack(s)"), NULL, gtkui_mitm_stop,     0, "<StockItem>", GTK_STOCK_STOP }
};

GtkItemFactoryEntry gmenu_filters[] = {
   {N_("/_Filters"),                 NULL,         NULL,              0, "<Branch>" },
   {N_("/Filters/Load a filter..."), "<control>f", gtkui_load_filter, 0, "<StockItem>", GTK_STOCK_OPEN },
   {N_("/Filters/Stop _filtering"),  "f",          gtkui_stop_filter, 0, "<StockItem>", GTK_STOCK_STOP }
};

GtkItemFactoryEntry gmenu_logging[] = {
   {N_("/_Logging"),                             NULL, NULL,            0, "<Branch>" },
   {N_("/Logging/Log all packets and infos..."), "<shift>I", gtkui_log_all, 0, "<StockItem>", GTK_STOCK_SAVE },
   {N_("/Logging/Log only infos..."),            "i",  gtkui_log_info,  0, "<StockItem>", GTK_STOCK_SAVE_AS },
   {N_("/Logging/Stop logging infos"),           NULL, gtkui_stop_log,  0, "<StockItem>", GTK_STOCK_STOP },
   {"/Logging/sep1",                         NULL, NULL,            0, "<Separator>" },
   {N_("/Logging/Log user messages..."),         "m",  gtkui_log_msg,   0, "<StockItem>", GTK_STOCK_REVERT_TO_SAVED },
   {N_("/Logging/Stop logging messages"),        NULL, gtkui_stop_msg,  0, "<StockItem>", GTK_STOCK_STOP },
   {"/Logging/sep2",                         NULL, NULL,            0, "<Separator>" },
   {N_("/Logging/Compressed file"),              NULL, toggle_compress, 0, "<ToggleItem>" }
};

GtkItemFactoryEntry gmenu_plugins[] = {
   {N_("/_Plugins"),                   NULL,         NULL,              0, "<Branch>" },
   {N_("/Plugins/Manage the plugins"), "<control>p", gtkui_plugin_mgmt, 0, "<StockItem>", GTK_STOCK_EXECUTE },
...
GtkItemFactoryEntry tab_menu[] = {
  ...
  { N_("/Next page"),      "<control>0", gtkui_page_right,    0, "<StockItem>", GTK_STOCK_GO_FORWARD },
  { N_("/Previous page"),  "<control>9", gtkui_page_left,     0, "<StockItem>", GTK_STOCK_GO_BACK }
};
